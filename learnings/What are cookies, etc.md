...
#### 3. Tokens (Stateless Authentication & DevOps Access)

Tokens are strings of data generated by a server that act as credentials for a client. They are often **stateless**, meaning the server does not need to store the token in a database to validate it; it just verifies the token's cryptographic signature. 

- **Role in GitHub/DevOps:** This is the primary method for machine-to-machine authentication.
- **Personal Access Tokens (PATs):** Used when using `git` via command line (HTTPS) instead of password.
- **OAuth Tokens:** Used by third-party services (e.g., Jira, CircleCI, Slack) to access GitHub APIs without knowing your password.
- **GitHub App Installation Tokens:** Used by GitHub Actions or automation scripts to act on behalf of an app rather than a user.
- **Stateless Nature:** Unlike sessions, once a token is issued, it cannot be easily revoked until it expires (unless a central database is checked, which negates the speed benefit).
- **DevOps Example:** A Jenkins server uses a GitHub Token to pull code from a private repository. 
